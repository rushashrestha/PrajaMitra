...
const FormDisplay = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        setFormData({ ...formData, submitted: true, appNumber: Math.floor(100000 + Math.random() * 900000) });
    };
        ...
        <div className="container">
            {!formData.submitted ? (
                <form onSubmit={handleSubmit} className="form-container">
                    <h2>{type === "citizenship" ? "Citizen Registration Issuance" : "Birth Registration Issuance"}</h2>
                    ...
                    <div className="form-group">
                        ...
                        <input type="date" name="regDate" onChange={handleChange} required />
                    </div>
                    <div className="form-group">
                        <label>Select Visit Date</label>
                        <input type="date" name="visitDate" onChange={handleChange} required />
                    </div>
                    {type === "citizenship" && (
                        <div className="form-group">
                ...
                <div className="application-container">
                    ...
                    <p><strong>Application Number:</strong> {formData.appNumber}</p>
                    ...
                    <p><strong>Citizenship No.:</strong> {formData.citizenship}</p>
                    <p><strong>Address:</strong> {formData.address}</p>
                    <p><strong>Date of Registration:</strong> {formData.regDate}</p>
                    <p><strong>Visit Date:</strong> {formData.visitDate}</p>
                    {type === "citizenship" && formData.photo && (
                        <div className="form-group">
                            <label>Uploaded Photo</label>
                            <img src={formData.photo} alt="Uploaded" className="uploaded-photo" />
                        </div>
                    )}
                    <p>The hard copy of this application must be submitted to the nearest office on the selected date.</p>
                </div>
            )}
        </div>
    );
